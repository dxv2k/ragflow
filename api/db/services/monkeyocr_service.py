...
class MonkeyOCRService(CommonService):
    ...
    def register_parser(cls, tenant_id: str) -> bool:
        ...
        try:
            ...
            tenant = Knowledgebase.select().where(Knowledgebase.tenant_id == tenant_id).first()
    ...
    @classmethod
    def parse_document(cls, doc_id: str, file_path: str, output_dir: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """Parse document using MonkeyOCR"""
        try:
            # Create parser instance
            parser = cls.create_parser()
            if not parser:
                return {"success": False, "error": "Failed to create MonkeyOCR parser"}

            # Parse document
            result = parser.parse_document(file_path=file_path, output_dir=output_dir, **kwargs)
