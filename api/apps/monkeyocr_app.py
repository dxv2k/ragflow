...
# Create Blueprint
monkeyocr_bp = Blueprint('monkeyocr', __name__, url_prefix='/api/v1/monkeyocr')

@monkeyocr_bp.route('/info', methods=['GET'])
@login_required
def get_parser_info():
...
@monkeyocr_bp.route('/register', methods=['POST'])
@login_required
def register_parser():
    ...
    try:
        ...
        return jsonify({
            'success': success,
            'message': 'Parser registered successfully' if success else 'Failed to register parser'
        })
...
@monkeyocr_bp.route('/unregister', methods=['POST'])
@login_required
def unregister_parser():
    ...
    try:
        ...
        return jsonify({
            'success': success,
            'message': 'Parser unregistered successfully' if success else 'Failed to unregister parser'
        })
...
@monkeyocr_bp.route('/available/<tenant_id>', methods=['GET'])
@login_required
...
@monkeyocr_bp.route('/parse', methods=['POST'])
@login_required
def parse_document():
    ...
    try:
        ...
        try:
            ...
            # Parse document
            result = MonkeyOCRService.parse_document(
                doc_id=doc_id,
                file_path=file_path,
...
@monkeyocr_bp.route('/extract-text', methods=['POST'])
@login_required
...
@monkeyocr_bp.route('/supported-formats', methods=['GET'])
@login_required
...
@monkeyocr_bp.route('/validate-file', methods=['POST'])
@login_required
def validate_file():
...
@monkeyocr_bp.route('/parsing-options', methods=['GET'])
@login_required
...
@monkeyocr_bp.route('/health', methods=['GET'])
def health_check():
...
def init_monkeyocr_app(app):
    """Initialize MonkeyOCR app with Flask app"""
    app.register_blueprint(monkeyocr_bp)
    logger.info("MonkeyOCR API endpoints registered") 