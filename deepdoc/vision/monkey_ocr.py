...
class MonkeyOCRProcessor:
    ...
    def parse_document(self, 
        ...
        if isinstance(infer_result, list):
            ...
            for page_idx, page_infer_result in enumerate(infer_result):
                self._process_page_result(
                    page_infer_result, page_idx, name_without_suff, 
                    output_dir, self.model
                )
        else:
            logger.info("Processing as single result...")
            self._process_single_result(
                infer_result, name_without_suff, local_md_dir, 
                image_writer, md_writer, image_dir
            )
            
        logger.info(f"Document parsing completed. Results saved to: {local_md_dir}")
        return str(local_md_dir)
    
    def _process_page_result(self, page_infer_result, page_idx, name_without_suff, 
                           output_dir, model):
    ...
    def _process_single_result(self, infer_result, name_without_suff, local_md_dir, 
                              image_writer, md_writer, image_dir):
    ...
    def extract_text_from_images(self, image_paths: List[Union[str, Path]], 
