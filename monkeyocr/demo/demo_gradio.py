...
if __name__ == "__main__":
    ...
    """
            ...
            result = subprocess.run(["pdflatex", "-interaction=nonstopmode", "-output-directory", temp_dir, tex_path], timeout=20, capture_output=True, text=True)
    ...
    instruction_mf = """Please write out the expression of the formula in the image using LaTeX format."""
    instruction_table_html = """This is the image of a table. Please output the table in html format."""
    instruction_table_latex = """Please output the table in the image in LaTeX format."""
    ...
    with gr.Blocks(theme="ocean", css=css, title="MonkeyOCR") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1, variant="compact"):
                ...
                chat_input = gr.Dropdown(
                    label="Select Prompt (选择Prompt)",
                    choices=[instruction, instruction_mf, instruction_table_html, instruction_table_latex],
                    value=instruction,
                    show_label=True,
                    multiselect=False,
                    visible=True,
                    ...
                    with gr.Column(scale=3):
                        ...
                        with gr.Tabs(elem_id="markdown_tabs"):
                            with gr.TabItem("Markdown Render Preview (Markdown渲染预览)"):
                                md_view = gr.Markdown(
                                    value="## Please click the parse button to parse or click chat for single-task recognition...",
                                    label="Markdown Preview (Markdown预览)",
                                    max_height=600,
                                    latex_delimiters=[
                            ...
                            with gr.TabItem("Markdown Raw Text (Markdown原始文本)"):
                                md_raw = gr.Textbox(
                                    ...
                                    label="Markdown Raw Text (Markdown原始文本)",
                                    max_lines=100,
                                    lines=38,
        ...
        pdf_input.upload(fn=load_file, inputs=pdf_input, outputs=[pdf_view, page_info])
        ...
        parse_button.click(
            fn=parse_and_update_view, inputs=pdf_input, outputs=[pdf_view, md_view, md_raw, page_info, pdf_download_button, md_download_button], show_progress=True, show_progress_on=[md_view, md_raw]
        )

        # Q&A button
        chat_button.click(
            fn=chat_with_image, inputs=[chat_input, pdf_input], outputs=[md_view, md_raw, pdf_download_button, md_download_button], show_progress=True, show_progress_on=[md_view, md_raw]
        )

        # Clear button
        clear_button.click(fn=clear_all, outputs=[pdf_input, pdf_view, md_view, md_raw, page_info, pdf_download_button, md_download_button], show_progress=False)
