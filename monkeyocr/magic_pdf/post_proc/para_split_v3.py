...
def __is_list_or_index_block(block):
    if len(block["lines"]) >= 2:
        ...
        for line in block["lines"]:
            ...
            if line["bbox"][0] - block["bbox_fs"][0] > 0.7 * line_height and block["bbox_fs"][2] - line["bbox"][2] > 0.7 * line_height:
        ...
        if (left_close_num / len(block["lines"]) >= 0.8 or right_close_num / len(block["lines"]) >= 0.8) and line_num_flag:
            for line in block["lines"]:
                line[ListLineTag.IS_LIST_START_LINE] = True
            return BlockType.Index

        elif external_sides_not_close_num >= 2 and center_close_num == len(block["lines"]) and external_sides_not_close_num / len(block["lines"]) >= 0.5 and block_height / block_weight > 0.4:
            for line in block["lines"]:
                line[ListLineTag.IS_LIST_START_LINE] = True
            return BlockType.List

        elif (
            left_close_num >= 2 and (right_not_close_num >= 2 or line_end_flag or left_not_close_num >= 2) and not multiple_para_flag
        ...
        if len(text_blocks_group) > 1:
            ...
            for i in range(len(text_blocks_group) - 1, -1, -1):
                ...
                if i - 1 >= 0:
                    ...
                    elif (current_block["type"] == BlockType.List and prev_block["type"] == BlockType.List) or (current_block["type"] == BlockType.Index and prev_block["type"] == BlockType.Index):
