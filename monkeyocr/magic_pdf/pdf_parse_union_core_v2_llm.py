...
from magic_pdf.pre_proc.ocr_span_list_modify import get_qa_need_list_v2, remove_overlaps_low_confidence_spans, remove_overlaps_min_spans, check_chars_is_overlap_in_span
...
def calculate_char_in_span(char_bbox, span_bbox, char, span_height_radio=0.33):
    ...
    if span_bbox[0] < char_center_x < span_bbox[2] and span_bbox[1] < char_center_y < span_bbox[3] and abs(char_center_y - span_center_y) < span_height * span_height_radio:
        return True
    else:
...
def txt_spans_extract_v2(pdf_page, spans, all_bboxes, all_discarded_blocks, lang):
    ...
    if len(empty_spans) > 0:
        ...
        ocr_model = atom_model_manager.get_atom_model(atom_model_name="ocr", ocr_show_log=False, det_db_box_thresh=0.3, lang=lang)
...
def do_predict(boxes: List[List[int]], model) -> List[int]:
    from magic_pdf.model.sub_modules.reading_oreder.layoutreader.helpers import boxes2inputs, parse_logits, prepare_inputs
...
def sort_lines_by_model(fix_blocks, page_w, page_h, line_height, MonkeyOCR_model):
    ...
    for block in fix_blocks:
        if block["type"] in [BlockType.Text, BlockType.Title, BlockType.ImageCaption, BlockType.ImageFootnote, BlockType.TableCaption, BlockType.TableFootnote]:
            if len(block["lines"]) == 0:
    ...
    for left, top, right, bottom in page_line_list:
        if left < 0:
            logger.warning(f"left < 0, left: {left}, right: {right}, top: {top}, bottom: {bottom}, page_w: {page_w}, page_h: {page_h}")  # noqa: E501
            left = 0
        if right > page_w:
            logger.warning(f"right > page_w, left: {left}, right: {right}, top: {top}, bottom: {bottom}, page_w: {page_w}, page_h: {page_h}")  # noqa: E501
            right = page_w
        if top < 0:
            logger.warning(f"top < 0, left: {left}, right: {right}, top: {top}, bottom: {bottom}, page_w: {page_w}, page_h: {page_h}")  # noqa: E501
            top = 0
        if bottom > page_h:
            logger.warning(f"bottom > page_h, left: {left}, right: {right}, top: {top}, bottom: {bottom}, page_w: {page_w}, page_h: {page_h}")  # noqa: E501
            bottom = page_h
        ...
        assert 1000 >= right >= left >= 0 and 1000 >= bottom >= top >= 0, f"Invalid box. right: {right}, left: {left}, bottom: {bottom}, top: {top}"  # noqa: E126, E121
...
def parse_page_core(page_doc: PageableData, magic_model, page_id, pdf_bytes_md5, imageWriter, parse_mode, lang, MonkeyOCR_model):
    need_drop = False
    drop_reason = []
    ...
    img_body_blocks, img_caption_blocks, img_footnote_blocks = process_groups(img_groups, "image_body", "image_caption_list", "image_footnote_list")

    table_body_blocks, table_caption_blocks, table_footnote_blocks = process_groups(table_groups, "table_body", "table_caption_list", "table_footnote_list")
    ...
    def merge_title_blocks(blocks, x_distance_threshold=0.1 * page_w):
        ...
        while title_bs:
            ...
            for block2 in title_bs:
                if __is_overlaps_y_exceeds_threshold(block1["bbox"], block2["bbox"], 0.9) and len(block1["lines"]) == 1 and len(block2["lines"]) == 1:
    ...
    spans = ocr_cut_image_and_table(spans, page_doc, page_id, pdf_bytes_md5, imageWriter)
...
def pdf_parse_union(
    ...
    for page_id, page in enumerate(dataset):
        ...
        if start_page_id <= page_id <= end_page_id:
            page_info = parse_page_core(page, magic_model, page_id, pdf_bytes_md5, imageWriter, parse_mode, lang, MonkeyOCR_model)
        else:
            page_info = page.get_page_info()
            page_w = page_info.w
            page_h = page_info.h
            page_info = ocr_construct_page_component_v2([], [], page_id, page_w, page_h, [], [], [], [], [], True, "skip page")
