...
class BatchAnalyzeLLM:
    ...
    def __call__(self, images: list, split_pages: bool = False, pred_abandon: bool = False) -> list:
            ...
            for image_index, useful_list in modified_images:
                for res in images_layout_res[image_index]:
                    for i in range(len(res["poly"])):
                        if i % 2 == 0:
                            res["poly"][i] = res["poly"][i] - useful_list[0] + useful_list[2]
                        else:
                            res["poly"][i] = res["poly"][i] - useful_list[1] + useful_list[3]
        ...
        logger.info(f"layout time: {round(time.time() - layout_start_time, 2)}, image num: {len(images)}")
        ...
        if split_pages or len(images) == 1:
            ...
            if pages_to_process_directly:
                ...
                for i, page_idx in enumerate(pages_to_process_directly):
                    ...
                    pre_res = {"category_id": 200, "score": 1.0, "poly": [0, 0, width, 0, width, height, 0, height]}
                    single_res = {"category_id": 15, "score": 1.0, "text": direct_results[i], "poly": [0, 0, width, 0, width, height, 0, height]}
        ...
        logger.info(f"llm ocr time: {round(time.time() - llm_ocr_start, 2)}, image num: {len(images)}")
