...
class MonkeyOCR:
    def __init__(self, config_path):
        ...
        if not os.path.exists(models_dir):
            raise FileNotFoundError(f"Model directory '{models_dir}' not found. Please run 'python download_model.py' to download the required models.")
        ...
        if self.layout_model_name == MODEL_NAME.DocLayout_YOLO:
            ...
            if not os.path.exists(layout_model_path):
                raise FileNotFoundError(f"Layout model file not found at '{layout_model_path}'. Please run 'python download_model.py' to download the required models.")
        ...
        if chat_backend == "lmdeploy":
            ...
            self.chat_model = MonkeyChat_OpenAIAPI(url=api_config.get("url"), model_name=api_config.get("model_name"), api_key=api_config.get("api_key", None))
        else:
...
class MonkeyChat_LMDeploy:
    def __init__(self, model_path, engine_config=None):
        try:
            ...
            raise ImportError("LMDeploy is not installed. Please install it following: https://github.com/Yuliang-Liu/MonkeyOCR/blob/main/docs/install_cuda.md to use MonkeyChat_LMDeploy.")
...
class MonkeyChat_vLLM:
    def __init__(self, model_path):
        try:
            ...
            raise ImportError("vLLM is not installed. Please install it following: https://github.com/Yuliang-Liu/MonkeyOCR/blob/main/docs/install_cuda.md to use MonkeyChat_vLLM.")
        self.model_name = os.path.basename(model_path)
...
class MonkeyChat_transformers:
    def __init__(self, model_path: str, max_batch_size: int = 10, max_new_tokens=4096, device: str = None):
        try:
            ...
            raise ImportError("transformers is not installed. Please install it following: https://github.com/Yuliang-Liu/MonkeyOCR/blob/main/docs/install_cuda.md to use MonkeyChat_transformers.")
        self.model_name = os.path.basename(model_path)
        ...
        try:
            self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                ...
                attn_implementation="sdpa" if self.device.startswith("cuda") else "sdpa",
    ...
    def batch_inference(self, images: List[Union[str, Image.Image]], questions: List[str]) -> List[str]:
        ...
        for i in range(0, total_items, self.max_batch_size):
            ...
            logger.info(f"Processing batch {i // self.max_batch_size + 1}/{(total_items - 1) // self.max_batch_size + 1} (items {i + 1}-{batch_end})")
        ...
        for image, question in zip(images, questions):
            try:
                ...
                messages = [
                    {
                        "role": "user",
                        "content": [{"type": "input_image", "image_url": f"data:image/{img_type};base64,{img}"}, {"type": "input_text", "text": question}],
                    }
                ]
...
class MonkeyChat_LMDeploy_queue:
    ...
    def __init__(self, model_path, max_batch_size=32, queue_timeout=0.1, max_queue_size=1000, engine_config=None):
        try:
            ...
            raise ImportError("LMDeploy is not installed. Please install it following: https://github.com/Yuliang-Liu/MonkeyOCR/blob/main/docs/install_cuda.md")
    ...
    def _collect_batch_requests(self):
        ...
        with self.queue_lock:
            ...
            while len(batch_requests) < self.max_batch_size and time.time() - start_time < self.queue_timeout and self.request_queue:
    ...
    def _process_batch_requests(self, batch_requests):
        ...
        try:
            ...
            logger.info(f"Processed batch of {len(batch_requests)} requests in {processing_time:.2f}s ({len(batch_requests) / processing_time:.1f} req/s)")
        ...
        try:
            try:
                ...
                def run_async_in_thread():
                    ...
                    try:
                        return new_loop.run_until_complete(self.async_single_inference(image, question))
                    finally:
                        new_loop.close()
...
class MonkeyChat_vLLM_queue:
    ...
    def __init__(self, model_path, max_batch_size=64, queue_timeout=0.1, max_queue_size=1000):
        try:
            ...
            raise ImportError("vLLM is not installed. Please install it following: https://github.com/Yuliang-Liu/MonkeyOCR/blob/main/docs/install_cuda.md")
