...
from magic_pdf.libs.boxbase import _is_in, bbox_distance, bbox_relative_pos, calculate_iou
...
class MagicModel:
    def __fix_axis(self):
        for model_page_info in self.__model_list:
            ...
            horizontal_scale_ratio, vertical_scale_ratio = get_scale_ratio(model_page_info, self.__docs.get_page(page_no))
    ...
    def __tie_up_category_by_distance_v2(
        ...
        for i, obj in enumerate(objects):
            ...
            for j, sub in enumerate(subjects):
                ...
                if left:
                    if dis_by_directions["left"][i][1] > bbox_distance(obj["bbox"], sub["bbox"]):
                        dis_by_directions["left"][i] = [
                            j,
                            bbox_distance(obj["bbox"], sub["bbox"]),
                        ]
                if right:
                    if dis_by_directions["right"][i][1] > bbox_distance(obj["bbox"], sub["bbox"]):
                        dis_by_directions["right"][i] = [
                            j,
                            bbox_distance(obj["bbox"], sub["bbox"]),
                        ]
                if bottom:
                    if dis_by_directions["bottom"][i][1] > bbox_distance(obj["bbox"], sub["bbox"]):
                        dis_by_directions["bottom"][i] = [
                            j,
                            bbox_distance(obj["bbox"], sub["bbox"]),
                        ]
                if top:
                    if dis_by_directions["top"][i][1] > bbox_distance(obj["bbox"], sub["bbox"]):
                        dis_by_directions["top"][i] = [
                            j,
                            bbox_distance(obj["bbox"], sub["bbox"]),
                        ]

            if dis_by_directions["top"][i][1] != float("inf") and dis_by_directions["bottom"][i][1] != float("inf") and priority_pos in (PosRelationEnum.BOTTOM, PosRelationEnum.UP):
                RATIO = 3
                if abs(dis_by_directions["top"][i][1] - dis_by_directions["bottom"][i][1]) < RATIO * axis_unit:
            ...
            if dis_by_directions["left"][i][1] != float("inf") or dis_by_directions["right"][i][1] != float("inf"):
                if dis_by_directions["left"][i][1] != float("inf") and dis_by_directions["right"][i][1] != float("inf"):
                    if AXIS_MULPLICITY * axis_unit >= abs(dis_by_directions["left"][i][1] - dis_by_directions["right"][i][1]):
                        ...
                        if abs(left_sub_bbox_y_axis - l_y_axis) + dis_by_directions["left"][i][0] > abs(right_sub_bbox_y_axis - l_y_axis) + dis_by_directions["right"][i][0]:
            ...
            if dis_by_directions["top"][i][1] != float("inf") or dis_by_directions["bottom"][i][1] != float("inf"):
                if dis_by_directions["top"][i][1] != float("inf") and dis_by_directions["bottom"][i][1] != float("inf"):
                    if AXIS_MULPLICITY * axis_unit >= abs(dis_by_directions["top"][i][1] - dis_by_directions["bottom"][i][1]):
                        ...
                        if abs(top_bottom_x_axis - l_x_axis) + dis_by_directions["bottom"][i][1] > abs(bottom_top_x_axis - l_x_axis) + dis_by_directions["top"][i][1]:
            ...
            if left_or_right[1] != float("inf") or top_or_bottom[1] != float("inf"):
                if left_or_right[1] != float("inf") and top_or_bottom[1] != float("inf"):
                    if AXIS_MULPLICITY * axis_unit >= abs(left_or_right[1] - top_or_bottom[1]):
                        y_axis_bbox = subjects[left_or_right[0]]["bbox"]
                        x_axis_bbox = subjects[top_or_bottom[0]]["bbox"]

                        if abs((x_axis_bbox[2] - x_axis_bbox[0]) - l_x_axis) / l_x_axis > abs((y_axis_bbox[3] - y_axis_bbox[1]) - l_y_axis) / l_y_axis:
            ...
            ret.append(
                {
                    ...
                    "obj_bboxes": [{"score": objects[j]["score"], "bbox": objects[j]["bbox"]} for j in sub_obj_map_h[i]],
                    "sub_idx": i,
                }
            )
        return ret
    ...
    def get_imgs_v2(self, page_no: int):
        with_captions = self.__tie_up_category_by_distance_v2(page_no, 3, 4, PosRelationEnum.BOTTOM)
        with_footnotes = self.__tie_up_category_by_distance_v2(page_no, 3, CategoryId.ImageFootnote, PosRelationEnum.ALL)
        ret = []
    ...
    def get_tables_v2(self, page_no: int) -> list:
        with_captions = self.__tie_up_category_by_distance_v2(page_no, 5, 6, PosRelationEnum.UP)
        with_footnotes = self.__tie_up_category_by_distance_v2(page_no, 5, 7, PosRelationEnum.ALL)
        ret = []
        for v in with_captions:
            record = {
