...
class LMDeployPatcher:
    def __init__(self):
        ...
        if self.lmdeploy_path:
            self.flashattention_file = os.path.join(self.lmdeploy_path, "pytorch", "kernels", "cuda", "flashattention.py")
...
def main():
    ...
    parser.add_argument("action", choices=["patch", "restore", "status", "clean"], help="Action type: patch=apply patch, restore=restore original file, status=check status, clean=clean backup")
