...
def single_task_recognition(input_file, output_dir, MonkeyOCR_model, task):
    ...
    try:
        ...
        print("Single task recognition completed!")
        print(f"Task: {task}")
...
Usage examples:
  ...
  python parse.py input.pdf --pred-abandon            # Enable predicting abandon elements
    ...
    parser.add_argument("input_path", help="Input PDF/image file path or folder path")

    parser.add_argument("-o", "--output", default="./output", help="Output directory (default: ./output)")

    parser.add_argument("-c", "--config", default="model_configs.yaml", help="Configuration file path (default: model_configs.yaml)")

    parser.add_argument("-t", "--task", choices=["text", "formula", "table"], help="Single task recognition type (text/formula/table). Supports both image and PDF files.")

    parser.add_argument("-s", "--split_pages", action="store_true", help="Split the output of PDF pages into separate ones (default: False)")

    parser.add_argument("-g", "--group-size", type=int, help="Maximum total page count per group when processing folders (applies to all file types)")

    parser.add_argument("--pred-abandon", action="store_true", help="Enable predicting abandon elements like footer and header (default: False)")
    ...
    try:
        ...
        if os.path.isdir(args.input_path):
            ...
            result_dir = parse_folder(
                folder_path=args.input_path, output_dir=args.output, config_path=args.config, task=args.task, split_pages=args.split_pages, group_size=args.group_size, pred_abandon=args.pred_abandon
            )
            ...
            if args.task:
                result_dir = single_task_recognition(input_file=args.input_path, output_dir=args.output, MonkeyOCR_model=MonkeyOCR_model, task=args.task)
                print(f"\n✅ Single task ({args.task}) recognition completed! Results saved in: {result_dir}")
            else:
                result_dir = parse_file(input_file=args.input_path, output_dir=args.output, MonkeyOCR_model=MonkeyOCR_model, split_pages=args.split_pages, pred_abandon=args.pred_abandon)
