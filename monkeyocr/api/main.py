...
app = FastAPI(title="MonkeyOCR API", description="OCR and Document Parsing API using MonkeyOCR", version="1.0.0", lifespan=lifespan)
...
async def classify_omr_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        if file_ext_with_dot not in allowed_extensions:
            raise HTTPException(status_code=400, detail=f"Unsupported file type: {file_ext_with_dot}. Allowed: {', '.join(allowed_extensions)}")
        ...
        try:
            ...
            return OMRResponse(
                ...
                processing_details={"filename": file.filename, "file_size": len(content), "classification_result": image_type},
            )
        ...
        try:
            ...
            if image_type != "multiple_choice":
                return OMRResponse(
                    ...
                    image_type=image_type,
                    processing_details={"filename": file.filename, "file_size": len(content), "classification_result": image_type, "extraction_skipped": True},
                )
            ...
            if ratings is None:
                return OMRResponse(
                    ...
                    image_type=image_type,
                    processing_details={"filename": file.filename, "file_size": len(content), "classification_result": image_type, "extraction_failed": True},
                )
...
async def parse_document_enhanced(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not file.filename or not file.filename.lower().endswith(".pdf"):
            raise HTTPException(status_code=400, detail="Only PDF files are supported for enhanced parsing")
                ...
                try:
                    ...
                    if image_type == "multiple_choice":
                        ...
                        if ratings and any(r > 0 for r in ratings):
                            ...
                            omr_results["processed_images"].append({"filename": image_filename, "image_type": image_type, "ratings": ratings, "replacement_text": replacement_text})
                    ...
                    with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(image_filename)[1], prefix=f"ocr_temp_{unique_suffix}_") as ocr_temp:
                    ...
                    try:
                        ...
                        if ocr_result_files:
                            ...
                            if ocr_text:
                                ...
                                ocr_results["processed_images"].append({"filename": image_filename, "image_type": image_type, "extracted_text": ocr_text})
...
async def async_parse_file(input_file_path: str, output_dir: str, split_pages: bool = False):
    ...
    await process_inference_results_async(infer_result, output_dir, safe_name, local_image_dir, local_md_dir, image_dir, split_pages)

    return local_md_dir


async def process_inference_results_async(infer_result, output_dir, name_without_suff, local_image_dir, local_md_dir, image_dir, split_pages):
    """
    ...
    if isinstance(infer_result, list):
        ...
        for page_idx, page_infer_result in enumerate(infer_result):
            task = process_single_page_async(page_infer_result, page_idx, output_dir, name_without_suff)
            tasks.append(task)
        ...
        await process_single_result_async(infer_result, name_without_suff, local_image_dir, local_md_dir, image_dir)
...
async def async_single_task_recognition(input_file_path: str, output_dir: str, task: str):
    ...
    if supports_async and hasattr(monkey_ocr_model.chat_model, "async_batch_inference"):
        ...
        try:
            ...
            responses = await asyncio.get_event_loop().run_in_executor(None, monkey_ocr_model.chat_model.batch_inference, images, instructions)
    else:
...
async def parse_document_internal(file: UploadFile, split_pages: bool = False):
    ...
    try:
        ...
        try:
            ...
            return ParseResponse(success=True, message=f"{file_type} parsing ({parse_type}) completed successfully", output_dir=result_dir, files=files, download_url=download_url)
...
def classify_image(image_path):
    ...
    is_multiple_choice = valid_circles >= 3 and valid_circles <= 50 and has_regular_pattern and circular_contours > text_contours and circular_contours >= 3
...
async def perform_ocr_task(file: UploadFile, task_type: str) -> TaskResponse:
    ...
    try:
        ...
        try:
            ...
            return TaskResponse(success=True, task_type=task_type, content=content, message=f"{task_type.capitalize()} extraction completed successfully")
        ...
        return TaskResponse(success=False, task_type=task_type, content="", message=f"OCR task failed: {str(e)}")
